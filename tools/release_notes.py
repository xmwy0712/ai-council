...
**一句话：修掉四个「配置看起来能用、其实不生效」的毛病，模型列表也能看到思考档位了。**

### 🆕 新功能

- **模型旁显示思考档位**：选模型时，每一项右边会标出它支持的档位（如 `high / low / max`）。
  不支持思考的会明确写「无思考档位」，不会留一片空白让你猜。
- **接入 DeepSeek 最新的 V4.1 Flash**：官方 9 月 10 日发布，模型名是 `deepseek-flash`，
  支持三档思考强度。旧的 `deepseek-v4-flash` 官方已下线，不再出现在列表里。

### 🔧 修复

- **修好了「长问题必然超时」**：以前模型思考稍久就会被判定超时，报「连接或读取超时」。
  原因是把 30 秒的「连接超时」误当成了「读取超时」——推理模型想久一点就必挂。
  而且配置里的「卡死判定超时」根本没接上，怎么调都没用；现在真的生效了。
- **修好了「换了模型还是报错」**：换模型时没有一起切换调用通道，新模型名被发到了
  旧厂商那边，于是报「限流 429」。现在换模型会自动切到它所属的厂商。
- **修好了评审/辩论的字数上限**：配置里写着上限、界面也提示「字数有上限」，
  但那个数字从没传给模型——设了等于没设。现在真的会约束。
- **修好了会话页显示 `settings.raw.off` 这种键名**的界面瑕疵。

### 🎨 界面

- **「单次调用上限」可以在网页上调了**：设置 → 运行参数，范围 60–7200 秒。
  超大会话（长上下文 + 高思考档位）建议放宽到 1800–3600 秒。
- **「换模型」「换适配器」改成下拉选择**：以前要手打模型名/适配器名，现在直接选。
""",
    "1.3.0": """## 这次更新了什么
